/**
 * Knowledge Base content — faithful TypeScript port of the design source
 * `Ramble IDP Hub MVP Design/docs-data.js` (RDH_DOCS).
 *
 * Docs are component-based: a list of typed blocks.
 * Block types: p · h2 · h3 · callout · code · steps · table ·
 * diagram (flow | sequence) · divider
 *
 * Every string (copy, code, markdown, specs) is verbatim from the source.
 */
import type { DiagramSpec } from './diagram-engine';

export type CalloutTone = 'info' | 'warning' | 'success' | 'ai';

export type Block =
  | { t: 'p'; md: string }
  | { t: 'h2' | 'h3'; text: string }
  | { t: 'callout'; tone: CalloutTone; title: string; md: string }
  | { t: 'code'; file?: string; lang?: string; code: string }
  | { t: 'steps'; items: { title: string; md: string }[] }
  | { t: 'table'; head: string[]; rows: string[][] }
  | { t: 'diagram'; title: string; spec: DiagramSpec }
  | { t: 'divider' };

export type BadgeTone = 'cyan' | 'heath' | 'grass';
...
export const CATS: string[] = ['All', 'Golden paths', 'Architecture', 'Authoring'];
